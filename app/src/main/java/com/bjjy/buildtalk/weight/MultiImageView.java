    ...
    public void setList(List<String> lists) throws IllegalArgumentException {
        ...
        if (MAX_WIDTH > 0) {
            if (imagesList.size() == 1){
                pxMoreWandH = (2 * (MAX_WIDTH - pxImagePadding * 1)) / 3; //解决右侧图片和内容对不齐问题
                pxOneMaxWandH = (2* MAX_WIDTH * 2) / 3;
            }else {
                pxMoreWandH = (MAX_WIDTH - pxImagePadding * 1) / 3; //解决右侧图片和内容对不齐问题
...
//            }
    ...
    private ImageView createImageView(int position, final boolean isMultiImage) {
        ...
        final ImageView imageView = new ImageView(mContext, null);
...
//            Glide.with(mContext).asBitmap().load(urlStr).into(new SimpleTarget<Bitmap>() {
//                @Override
//                public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
//                    int width = resource.getWidth();
//                    int height = resource.getHeight();
//                    if (width < height && height > AllUtils.getmPicSizeLong()){
////                        imageView.setStyleType(YLCircleImageView.TOP);
